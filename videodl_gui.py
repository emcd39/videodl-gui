...
class VideoDLGUI:
    def __init__(self, root):
        ...
        self.config_file = os.path.join(
            os.path.expanduser("~"), "Documents", "videodl_config.json"
        )
    ...
    def setup_ui(self):
        ...
        title_label = tk.Label(
            main_frame, text="videodl 视频下载工具", font=("Arial", 16, "bold")
        )
        ...
        tk.Button(url_frame, text="粘贴", command=self.paste_url, width=8).pack(
            side=tk.LEFT
        )
        ...
        default_dir = (
            ...
            else self.get_default_download_dir()
        )
        self.download_dir = tk.StringVar(value=default_dir)

        dir_entry = tk.Entry(
            dir_frame, textvariable=self.download_dir, font=("Arial", 10)
        )
        dir_entry.pack(side=tk.LEFT, fill=tk.X, expand=True, ipady=5, padx=(0, 5))
        tk.Button(dir_frame, text="浏览", command=self.browse_directory, width=8).pack(
            side=tk.LEFT
        )
        ...
        self.log_text = scrolledtext.ScrolledText(
            log_frame, font=("Consolas", 9), wrap=tk.WORD, height=10
        )
    ...
    def paste_url(self):
        ...
        try:
            ...
            if url:
                ...
                self.root.after(
                    3000, lambda: self.status_label.config(text="就绪", fg="#7f8c8d")
                )
            else:
        ...
        try:
            ...
            if not os.path.exists(download_dir):
                try:
                    ...
                    self.log(f"创建下载目录: {download_dir}", "info")
                except Exception as e:
                    self.log(f"无法创建目录: {e}", "error")
                    raise
